...
lv_obj_t *lv_calendar_create(lv_obj_t *par, const lv_obj_t *copy)
{
    ...
    if (ancestor_signal == NULL)
        ancestor_signal = lv_obj_get_signal_cb(calendar);
    if (ancestor_design == NULL)
        ancestor_design = lv_obj_get_design_cb(calendar);
 ...
 *=====================*/
void lv_calender_left(lv_obj_t *obj)
{
    lv_calendar_signal(obj, LV_SIGNAL_CONTROL, ((uint8_t *)LV_KEY_LEFT));
}

void lv_calender_right(lv_obj_t *obj)
{
    lv_calendar_signal(obj, LV_SIGNAL_CONTROL, ((uint8_t *)LV_KEY_RIGHT));
}
...
static lv_res_t lv_calendar_signal(lv_obj_t *calendar, lv_signal_t sign, void *param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE)
    {
        ...
        if (info->result != NULL)
            ...
            return ancestor_signal(calendar, sign, param);
        return LV_RES_OK;
    }
...
static void draw_header(lv_obj_t *calendar, const lv_area_t *mask)
{
    ...
    /*Add the left arrow*/
    if (ext->btn_pressing < 0)
        calendar->state |= LV_STATE_PRESSED;
    else
        calendar->state &= ~(LV_STATE_PRESSED);
...
static bool is_pressed(lv_obj_t *calendar, day_draw_state_t draw_state, int32_t year, int32_t month, int32_t day)
{
    ...
    if (year == ext->pressed_date.year && month == ext->pressed_date.month && day == ext->pressed_date.day)
        return true;
    else
        return false;
}
